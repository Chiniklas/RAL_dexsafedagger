...
SafeDAgger with a bi-channel stationary intervention policy, and
\methodname---in terms of growth in lift-success rate, unsafe-episode rate, and intervention
...
This is the stationary variant of \methodname: the teacher is trained with our customized RL recipe (as in (b)), and distillation
...
Comparing ablations (e) and (f) isolates the contribution of VGIP: both use the same RL
teacher and disagreement--risk gate, but (e) uses fixed thresholds, whereas (f) adapts them
using rollout statistics and visual context. Ablation (f) achieves consistently lower unsafe
rates across all evaluated objects while maintaining comparable lift success, demonstrating
that VGIP improves safety without sacrificing task performance.
