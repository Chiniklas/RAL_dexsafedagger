...
\begin{figure*}[t]
    ...
    stationary intervention policy (green), and \methodname (red).}
    \label{fig:ablation}
\end{figure*}
...
The following ablation studies validate the design choices in \methodname. We distill the teacher policies
trained during the RL phase into a compact, generalizable student policy. For RL training, we compare the
standard DexTrAH-RGB pipeline with our customized variant. For skill distillation, we evaluate four
methods---Vanilla DAgger, Vanilla SafeDAgger, \revision{stationary intervention policy,
and \methodname}---in terms of lift-success growth, unsafe-episode rate, and intervention rate
...
As our primary metric, we report the unsafe-episode rate averaged across the four ablation objects and
decomposed by object and failure mode. The ablation settings are defined below.
...
\noindent\textbf{(d) DexTrAH-RGB RL + SafeDAgger with disagreement and a risk predictor under a stationary intervention policy.}
This setting uses a stationary intervention policy with the DexTrAH-RGB-trained teacher (as in (a)).
...
\noindent\textbf{(e) Our RL + SafeDAgger with disagreement and a risk predictor under a stationary intervention policy.}
\revision{This is the stationary variant of \methodname:} the teacher is trained with our customized RL recipe (as in (b)), and distillation
uses the same intervention gate as in (d).

\noindent\revision{\textbf{(f) Our RL + \methodname.}
...
Compared with Vanilla SafeDAgger, \methodname's risk predictor triggers more teacher takeovers and lowers
the unsafe rate by about 5 percentage points on average. By the end of training, its average unsafe rate
approaches the teacher's average ($\approx 12\%$), reaching a minimum of approximately 8\%.
...
\begin{figure*}[t]
    ...
    yellow, Vanilla SafeDAgger\cite{zhang2016query}; green, stationary intervention policy;
    and red, \methodname.}
...
Comparing ablation (c), which uses disagreement alone, with ablation (e), which adds risk prediction,
shows that the combined gate reduces collision and escape failures missed by disagreement alone.
Ablation (f), the full \methodname configuration, achieves the lowest unsafe rates overall, indicating that VLM-guided
policy adaptation further improves the intervention gate while preserving competitive task performance.
