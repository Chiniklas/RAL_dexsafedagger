...
For $N$ evaluation episodes of 10\,s, let $\tau$ index episodes and $t$ timesteps.

\paragraph{Lift success (ever-lifted with hold).}
For object height $h_t^\tau$, lift threshold $H$, and hold duration $N_{\mathrm{hold}}$, define
...
Let $\mathcal{M}=\{\mathrm{coll},\mathrm{obj},\mathrm{flip},\mathrm{hf}\}$
denote harmful collision, object escape, palm flip, and hand-too-far, and let
$\mathbb{I}_m^\tau$ indicate mode $m$ in episode $\tau$. Define
$\mathbb{I}^{\tau}_{\mathrm{unsafe}}
=\mathbb{I}\!\left[\bigvee_{m\in\mathcal{M}} \mathbb{I}^{\tau}_{m}=1\right]$
and report
$\mathrm{UnsafeRateAvg}=\frac{1}{N}\sum_{\tau=1}^{N} \mathbb{I}^{\tau}_{\mathrm{unsafe}}.$

\paragraph{Failure-mode decomposition.}
Each episode is assigned its first unsafe terminal mode
$m^\tau\in\mathcal{M}$; a fixed priority resolves simultaneous triggers. We define
...
space is 13-D (7D arm + 6D hand). The RGB student receives two $320\times240$ streams at 30\,fps, encoded by
ResNet-18 and fused into a 128-D actor feature. Training and distillation use Isaac Sim 5.0.0/Isaac Lab 2.2.1 on
one RTX~5090 GPU.
...
\dexsafedagger, and \methodname---in terms of the evolution of lift-success, unsafe-episode, and intervention
...
\dexsafedagger denotes SafeDAgger with disagreement and a Bellman success-value critic under a stationary intervention policy.
This setting uses the DexTrAH-RGB-trained teacher (as in (a)).
Intervention is triggered by either large teacher--student disagreement or low predicted success,
...
\begin{figure*}[!t]
    ...
    \caption{\textbf{Per-object failure-mode decomposition during online distillation.}
    For \texttt{WindCart} and \texttt{Cow}, we show the aggregate unsafe-episode rate and its breakdown into
    harmful collision, object escape, palm flip, and hand-too-far over training; lower is better.
    Blue: Vanilla DAgger\cite{ross2011reduction}; orange: Vanilla SafeDAgger\cite{zhang2016query};
    green: \dexsafedagger; red: \methodname.}
...
Both use the same RL teacher and two-channel intervention policy, but (e) uses fixed thresholds, whereas
(f) adapts them using a VLM conditioned on rollout statistics and visual context. Across all evaluated
objects, ablation (f) achieves an unsafe-episode rate that is lower than that of (e) by an average of
4 percentage points, while maintaining comparable lift success. This result shows that VLM-guided intervention
policy adaptation improves safety without sacrificing task performance.

Compared with ablation (c), which uses disagreement alone, ablation (e) adds success-value prediction
...
\begin{figure}[!t]
    ...
    \caption{\textbf{Real-world setup and test objects.}
    Left: Tiangong~2 Pro with an InspireHand RH56DfX and an external stereo camera. Right: test objects.}
