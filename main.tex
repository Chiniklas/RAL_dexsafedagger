...
\title{\LARGE \bfseries
VIBS: Visual-Semantic Intervention Boundary Shaping for Safety-Critical Visuomotor Policy Distillation
...
\renewcommand{\@maketitle}{%
  ...
  \begin{center}
    ...
    intervention mechanism that combines teacher--student action disagreement with a
    success-value critic trained from lift-success labels.}
    \label{fig:teaser}
...
To address this challenge, we propose a safety-aware online policy distillation framework that transfers reinforcement learning (RL) teacher policies to a vision-based student policy. VC-DexSafeDAgger combines a SafeDAgger teacher--student disagreement gate with a Bellman critic trained from lift-success labels. The critic estimates the future success value of the student action, and low predicted success proactively triggers teacher takeover. \revision{A vision-language model calibrates the disagreement and success thresholds without replacing the success critic or directly selecting actions.} This design enables safer exploration while preserving the benefits of online interactive imitation learning.
