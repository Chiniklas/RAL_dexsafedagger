...
\newcommand{\revision}[1]{{\color{yellow!50!black}#1}}
...
\renewcommand{\@maketitle}{%
  ...
  \begin{center}
    \includegraphics[width=1.0\linewidth]{figures/fig01_teaser-dexsafedagger.pdf}
    \par\redtodo{TODO: Update Figure 1.}
...
To address this challenge, we propose \methodname (\methodfullname), a safety-aware online skill-distillation framework that transfers reinforcement learning (RL) teacher policies to a vision-based student policy. \methodname combines a SafeDAgger teacher--student disagreement gate with a Bellman critic trained from lift-success labels. The critic estimates the future success value of the student action, and low predicted success proactively triggers teacher takeover. \revision{A vision-language model adapts the intervention policy by calibrating the disagreement and success thresholds without replacing the success critic or directly selecting actions.} This design enables safer exploration while preserving the benefits of online interactive imitation learning.
Experiments on grasp-and-lift tasks show that \methodname significantly improves training safety, reducing unsafe episodes by up to 80\% during the early stages of training compared with vanilla DAgger-based distillation. Meanwhile, the distilled student policy achieves comparable lift success rates and demonstrates robustness suitable for real-world robotic deployment.
\end{abstract}
