...
The VLM is used only as a high-level calibration module. It recommends candidate
thresholds
$(\tilde{\tau}_{\delta}^{(k)},\tilde{\tau}_{Q}^{(k)})$. To avoid abrupt changes and preserve a bounded
safety envelope, each proposed threshold is constrained around its initial value
