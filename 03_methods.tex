\begin{figure*}[t]
    ...
    semantic displacement of the intervention operating point; deterministic
    clipping and smoothing constrain the resulting boundary update.}}
    \label{fig:pipeline}
\end{figure*}
...
The intervention boundary $\boldsymbol{\tau}^{(k)}=
(\tau_\delta^{(k)},\tau_Q^{(k)})$ governs the teacher takeover mechanism in $\mathcal{G}$. Periodically, a VLM proposes a displacement
$\Delta\boldsymbol{\tau}^{(k)}$, from which the boundary is updated to adjust to distributional shift.
...
(Sec.~E), (iv) visual-semantic intervention boundary calibration (Sec.~F), and (v) online
student distillation (Sec.~G).
...
the intervention operating point through a semantic boundary shift. The
resulting candidate boundary is clipped and smoothed before use. Pseudo-code is
...
Action disagreement identifies imitation errors but remains insufficient in
contact-rich manipulation tasks: even a small instantaneous deviation can destabilize
contact and later cause an unstable grasp, object escape, harmful collision, or
palm flipping. We therefore learn a success-value critic that
predicts the future probability of safe lift success under the student action,
providing a prospective signal complementary to $\ell_2$ action disagreement.
...
auxiliary object-position prediction loss (Sec.~G). At time $t$, $a_t$ denotes
...
$\hat Q_{\mathrm{succ}}(s_t,a_t^S)$. The intervention operating point is
...
takeover. Decreasing $\tau_\delta$ or increasing $\tau_Q$ makes the boundary
more conservative; the opposite shift increases student autonomy.
...
\subsection{Visual-Semantic Intervention Boundary Calibration}
\label{subsec:vlm_threshold_tuner}
\begingroup
\color{yellow!50!black}
During online distillation, the student policy, visited-state distribution, and
risk-predictor calibration co-evolve, while the dominant failure modes may also
change. A fixed intervention boundary therefore cannot maintain a stationary
safety--autonomy trade-off. We leverage a VLM periodically as a visual-semantic
calibrator that proposes an operating-point displacement.
...
From this context, it proposes the semantic boundary shift
...
Each candidate boundary parameter is clipped around its initial value and
