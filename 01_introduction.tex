...
manipulation behaviors~\cite{brahmbhatt2019contactgrasp}\cite{chen2022system}\cite{andrychowicz2020learning}\cite{luo2025precise}. In recent years, reinforcement learning (RL)-based approaches~\cite{rajeswaran2017learning,andrychowicz2020learning,akkaya2019solving,petrenko2023dexpbt} have made significant progress in addressing the complex control problems posed by high-dimensional action spaces and intricate contact dynamics in dexterous manipulation tasks.

Despite these advances, existing RL frameworks primarily optimize for task 
success rates and rarely incorporate explicit modeling or constraints related to safety. During training, common failure modes, including severe collisions between the hand and the environment, unintended object ejection due to excessive contact forces, and loss of stable hand posture~\cite{liu2023safe,yu2022dexterous}, not only reduce training efficiency but also introduce potential risks during real-world deployment. Therefore, how to 
...
%====================================================
% To mitigate the performance gap between simulation and real-world deployment, skill distillation has become a widely adopted paradigm in recent years. The core idea is to first train a high-performing expert policy in simulation using privileged state information, and then distill its decision-making behavior into a student policy that relies solely on visual observations or partially observable inputs. This process enables modality transfer from privileged state information to perception-based inputs, facilitating efficient real-world deployment.
% Skill distillation reduces reliance on accurate state estimation and allows knowledge integration from multiple teacher policies, thereby significantly improving execution stability and generalization performance in real-world environments. Consequently, it has gained increasing attention in dexterous grasping and complex manipulation tasks.
...
To bridge the sim-to-real gap, skill distillation is widely used to transfer the behavior of expert policies trained with privileged information to student policies capable of operating on real robots.
...
Typical safety-aware distillation approaches, such as SafeDAgger~\cite{zhang2016query}, have primarily focused on autonomous driving scenarios and have not been systematically studied in contact-rich tasks such as dexterous manipulation.
...
% transfer. Among these approaches, skill distillation has attracted
...
% Most prior work studies skill distillation purely in simulation, where it is
...
\begin{itemize}
    \item We propose \methodname, a VLM-guided,
    safety-sensitive framework for
    distilling RL-acquired teacher policies into a single RGB-based student
    policy. To the best of our knowledge, this is the first attempt in dexterous manipulation to incorporate
    a mixed intervention policy combining teacher--student action disagreement
    with a Bellman success-value critic, while the VLM reasons about scene-level safety from rollout statistics and
    representative visual observations to regulate the intervention gate.
    ...
    \item We empirically demonstrate that \methodname, which combines
    disagreement, success-value prediction, and VLM-guided intervention policy adaptation,
    substantially reduces unsafe exploration compared with Vanilla DAgger,
    achieving an approximately \textbf{80\%} relative reduction in the
    unsafe-episode rate during early training. It also reduces the average
    unsafe-episode rate by \textbf{7.5 percentage points} relative to
    disagreement-only SafeDAgger~\cite{zhang2016query}. Ablation results further demonstrate the
    effectiveness of VLM-guided adaptation across different visual contexts and
    unsafe scenarios: \methodname consistently achieves lower unsafe-episode
    rates than \dexsafedagger, with an average reduction of
    \textbf{4 percentage points}.
\end{itemize}
